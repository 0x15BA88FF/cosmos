...
{
  options.modules.home.tmux.enable = lib.mkEnableOption "Enable tmux";

  config = lib.mkIf config.modules.home.tmux.enable {
    programs.tmux = {
      enable = true;
      extraConfig = ''
        set -g set-titles on
    ...
    home.packages = import ./tmux/scripts { inherit pkgs; };
  };
}
