...
let
  mpvOscModern = pkgs.fetchzip {
    url = "https://github.com/maoiscat/mpv-osc-modern/archive/refs/tags/v1.1.1.tar.gz";
    sha256 = "sha256-RMUy8UpSRSCEPAbnGLpJ2NjDsDdkjq8cNsdGwsQ5ANU=";
  };
in
{
  options.modules.desktop.applications.mpv.enable = lib.mkEnableOption "Enable mpv";

  config = lib.mkIf config.modules.desktop.applications.mpv.enable {
